...
```python
traces = langfuse.api.trace.list(session_id="session-789")
```

**Fetch all observations in a trace**:
```python
observations = langfuse.api.observations_v_2.get_many(trace_id="trace-abc123")
```

**Filter traces by user**:
```python
traces = langfuse.api.trace.list(user_id="user-456")
```
